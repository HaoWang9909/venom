...
void launch_kernels(int pattern_code, int gemm_code, int spmm_code, int check, int m, int n, int k, float density, int block_sz, int meta_block_sz, int bm, int bn, int bk, int wm, int wn, int wk, int mm, int mn, int mk, int nstage, int nn_row, int mm_row, int seed, cudaDataType_t acc_type, cublasComputeType_t C_gemm, cusparseComputeType C_spmm){
    ...
    cout << spmm_code << "," << "sm_90" << "," << m << "," << k << "," << n << "," << meta_block_sz << "," << block_sz << "," << nn_row << "," << mm_row << "," << density << "," << bm << "," << bn << "," << bk << "," << wm << "," << wn << "," << wk << "," << mm << "," << mn << "," << mk << "," << nstage << "," << spmm_time << "," << gemm_time << "," << gemm_time/spmm_time << "," << error << endl;
...
int main(int argc, const char **argv) {
    ...
    {
        case 0:
            launch_kernels<float, float, float>(pattern_code, gemm_code, spmm_code, check, m, n, k, density, block_sz, meta_block_sz, bm, bn, bk, wm, wn, wk, mm, mn, mk, nstage, nn_row, mm_row, seed, CUDA_R_32F, CUBLAS_COMPUTE_32F, CUSPARSE_COMPUTE_32F);
            break;
        case 1:
